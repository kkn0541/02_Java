...
public class FileService {
	...
	public void method1() {
		...
		if(!directory.exists()) { //폴더가 존재하지 않으면 
			...
			System.out.println(directory.getPath()); // 윈도우이기 때문에 백슬래싀 \io_test/20240926
	...
	public void method02() {
		...
		// -> \n 줄바꿈 , \t 탭과 같이 특정 문자 뒤에 특별한 의미를 부여하는 역할
		...
		if(!file.exists()) { //존재하지 않으면 
			...
			try {
				...
				//File클래스의 메서드는 대부분 ioexception 발생시킴 
				//->ioexception은 반드시 예외처리를 처리하는 checked Exception 
...
public void method3() {
	...
	for(File f: files) {
		...
		//파일명 
		String fileName = f.getName(); // .class
		
		//마지막으로 수정한 날짜 
		long lastModified = f.lastModified();
		//System.out.println(lastModified);
		
		//java.text.SimpleDateFormat : 간단히 날짜 형식을 지정할수 있는 객체 
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mm");
													//2024-09-26 오전 11:17 
		
		//string simpledatefomat.format(long) : 
 ...
 * 즉 , 파일이나 폴더에 실제 데이터를 읽거나 쓸때 스트림을 이용함 
 * 하지만 폴더 / 파일 생성 및 삭제하는 것은 입출력 작업이 아닌 
 * 파일 시스템과의 일반적인 상호작용이기때문에 스트림 사용할 필요가 없음 
 * 
 * 
 * 
 * */
