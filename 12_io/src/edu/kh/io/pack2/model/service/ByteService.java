...
public class ByteService {
	...
	/*
	 * 스트림 (Stream) : 데이터가 이동하는 통로 기본적으로 한방향으로 흐름
	 * 
	 * Byte 기반 스트림
	 * 
	 ...
	 * 
	 * -문자열 , 이미지 , 영상 , 오디오 , pdf 등 모든것을 입/출력 할수 있음 (단, 통로가 좁다보니 속도가 좀 느림 , char 형
	 * 문자는 깨질우려가 있음 )
	 * 
	 * 
	 */
	...
	public void fileByteOutput() {
		...
		// FileOutputStream 참조 변수 선언 (OutputStream 을 상속 받은 자식클래스)
		FileOutputStream fos = null;
		try {

			// new FileOutputSTREAM("경로");
			// ->경로에 작성된 파일과 연결된 파일출력 스트림 객체 생성
			// ->출력 스트림에 연결된 파일이 존재하지 않으면 자동 생성
			// 단 , 폴더는 생성되지 않음
			// 기존 파일이 존재하면 내용을 덮어쓰기함

			// 자바프로그램이 내부 (연결 통로) 20240926 폴더
			 ...
			 * 
			 * char ch= content.charAt(i); //i번째 글자 반환 fos.write(ch); // 1byte 출력 스트림이 인결된
			 * 파일에 ch쓰기
			 * 
			 * }
			 * 
			...
			System.out.println("[수행시간] : " + (endTime - startTime) + "ns"); // 66500ns
	...
	/**
	 * 버퍼를 이용한 파일 출력 +BuffredOutputStream 보조 스트림 이용
	 */
	public void bufferedFileByteOutput() {
		 ...
		 * 
		 * 버퍼링 -인터넷 속도 낮음 + 스트리밍 코드가 효율이 안좋음 ->동영상 재생중에 미리 앞에 재생될 부분을 다운 받아놔서 영상을 끊김없이
		 * 볼수 잇게 함 ->다운 받아서 저장해 놓은 공간 == 버퍼
		 * 
		 * 
		 * BuffredOutputStream / BuffredInputStream 출력용 보조 스트림(바이트) / 입력용 보조 스트림(바이트)
		 * 
		 * FileOutputStream 은 1바이트씩 데이터를 입 출력 ->통로가 좁아서 데이터 입출력이 느림
		 * 
		 * ->보조스트림 이용 시 버퍼에 입출력할 내용을 모아서 한번에 입출력 --> 기반 스트림을 이용하는 횟수가 적어짐 -> 성능 , 시간 효율
		 * up!
		 * 
		 * 
		 * 
		...
		try {
			...
			fos = new FileOutputStream("/io_test/20240926/바이트기반_테스트_Buffred.txt");
			...
			System.out.println("[수행시간] : " + (endTime - startTime) + "ns"); // 27400ns
			// 보조 스트림 이용시 수행 시간이 좀더 빠르다 .
...
//보조스트림 (bos) close() 시 
	...
	/**
	 * 바이트 기반 파일 입력 외부에 있는걸 자바로 가져와서 출력
	 * 
	 */
	public void fileByteInput() {
		...
		try {
			...
			// inputStream 만들대는 해당 경로에 실제 파일이 있어야함

			// 방법 1. 파일 내부 내용을 1byte 씩 끊어오기
			...
			while (true) {
				...
				// 단 , 더이상 읽어올 값이 없으면 -1반환

				if (value == -1)
					break; // 다 일거왔으면 반복 멈춤

				sb.append((char) value); // char로 강제 형변환 해서 글자 형태로 sb추가
	...
	/**BuffredInputStream 보조 스트림을 이용한 성능향상 
	 * 
	 * inputStream의 경우 외부파일을 읽어올때 사용 
	 * ->읽어오는 파일의 크기가 작으면 보조스트림의 성능 이점이 크게 눈에 띄지 않음.
	 *
	 * 
	 */
	 ...
	 *  :FileOuputStream + BufferedOutputStream
	 *  
	 *  
	 *  +Sanner 대신 BuffredReader 이용 
	 *  
	 * 
	 * 
	 */
	public void fileCopy() {
		...
		//               보조                   문자기반	
			...
			//입력용 스트림을 이용해서 target파일의 내용 입력 받기 
			byte[] bytes = bis.readAllBytes();
