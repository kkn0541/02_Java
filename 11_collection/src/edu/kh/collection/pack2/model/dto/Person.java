...
public class Person {
	...
	// - 동일 : 가리키고 있는 것이 같다 (메모리 주소값이 같다)
	// - 동등 : 값이 같음 (메로리주소고가 달라도 객체 내 쓰여진 값이 같다)
	
	//동일 객체 : 메모리 주소가 같은 완전히 같은 객체 
	//				hashCode()는 주로 이런 객체를 식별할때 사용 
	
	//동등객체 : 메모리주소가 다르더라도 , 각 객체의 내부 값들이 같다면 
	// 			동일한 객체로 간주하는 경우를 말함 
